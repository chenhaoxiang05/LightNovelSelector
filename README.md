...
识别顺序综合自定义规则、本地修正记忆、文件名、本地 EPUB 元数据与内容提示，以及可选的 Bangumi、AniList、Jikan 在线条目。三个在线来源通过统一注册表按优先级运行，单个来源异常不会阻断后续来源；设置页会显示核心实际启用的来源。内部使用统一书籍身份记录书名、系列、作者、卷号、语言和标签；字段没有足够证据时会显示“未识别”，不会为了填满详情而强行猜测。不同来源的原始分数会经过统一的保守标尺，详情区同时展示置信度等级、分类原因和证据；选择候选只填写修正框，不会立即移动文件。
