...
识别顺序综合自定义规则、文件名、本地 EPUB 元数据与内容提示，以及可选的 Bangumi、AniList、Jikan 在线条目。内部使用统一书籍身份记录书名、系列、作者、卷号、语言和标签；字段没有足够证据时会显示“未识别”，不会为了填满详情而强行猜测。详情区先展示当前结果和本地候选，只有点击“联网查找更多”才会补充在线候选；选择候选只填写修正框，不会立即移动文件。
