...
- **识别结果可解释**：统一显示书名、系列、作者、卷号、语言、标签、识别来源和置信度，并可比较候选后手动修正。
...
- **工作区随窗口调整**：宽屏同时展示详情，紧凑窗口优先保留结果表格，并提供不中断浏览的详情入口。
...
识别顺序综合自定义规则、文件名、本地 EPUB 元数据与内容提示，以及可选的 Bangumi、AniList、Jikan 在线条目。三个在线来源通过统一注册表按优先级运行，单个来源异常不会阻断后续来源；设置页会显示核心实际启用的来源。内部使用统一书籍身份记录书名、系列、作者、卷号、语言和标签；字段没有足够证据时会显示“未识别”，不会为了填满详情而强行猜测。详情区先展示当前结果和本地候选，只有点击“联网查找更多”才会补充在线候选；选择候选只填写修正框，不会立即移动文件。
