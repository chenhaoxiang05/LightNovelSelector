...
缓存命名空间由按有效优先级排列的 `provider_id@cache_version:priority` 生成。同一查询在不同来源组合、
启用状态或用户优先级之间不会串用缓存。每条记录还会绑定实际返回结果的 `provider_id`；读取缓存时会确认该
...
所有来源调用都经过共享的 `ProviderReliabilityController`：

- 同一来源的请求至少间隔 150 毫秒，不同来源互不阻塞；
- 一次普通异常会让该来源冷却 30 秒，连续失败按 30、60、120、240、300 秒增长；
- 任一次成功会清空连续失败和冷却；
- 只有提供器正常返回 `None` 才对“来源 + 查询类型 + 查询摘要”建立 120 秒内存负缓存；
- 负缓存最多 4096 条，不写入磁盘，也不保存可还原的查询文本；
- 冷却、无结果或来源被用户禁用时，协调器会继续后续来源，全部不可用时回退本地规则。

提供器自身不要再叠加无限重试。确有服务端退避要求时，应让单次调用抛出普通异常，由共享控制器统一处理，
避免一个来源在内部占住整个扫描线程。
