...
12. 空闲窗口若存在未保存的识别设置，关闭前由用户选择保存、放弃或继续编辑；保存失败会保留现场并返回设置页。
...
- `WindowCloseGuardController` 以纯状态判定关闭动作，优先保护文件移动或撤销，其次阻止对话框重入，再处理未保存设置。
- 所有 `ContentDialog` 由页面级协调入口串行显示；退出确认单独标记生命周期，二次关闭不会创建重叠模态层。
...
- `Components/WorkflowRail` 只呈现流程阶段、紧凑状态和版本信息，不读取分类计划，也不发起文件操作。
- `Components/FileDetailPane` 统一呈现详情空白、加载、失败、重试、候选和修正状态；主页面通过事件接收用户意图，组件不直接调用 Sidecar。
- 宽屏内联详情与紧凑窗口侧栏共用同一个 `FileDetailPane` 实例，避免两套控件产生选择、加载和修正状态偏差。
- `FirstRunOnboardingController` 只决定当前引导版本是否已读，`ExperiencePreferences` 使用外观偏好相同的容错存储；持久化失败不会阻断工作台。
- 首次引导锚定目录选择按钮，只说明“先预览、再整理”，关闭后不再自动出现；帮助按钮可手动重新打开。
...
整理开始时，Python 先按规范化报告路径取得 Windows 命名互斥体，再以独占方式创建同目录的
`<报告名>.recovery.jsonl`，然后原子写入包含全部计划和唯一执行编号的基线报告。互斥体覆盖
GUI、CLI 和其他 Sidecar 进程，已有恢复日志会继续阻止新任务覆盖上次现场。每次实际移动前
只向恢复日志追加一条源路径和最终目标路径并 `fsync`；正常完成时统一写最终报告并删除日志。
因此 N 个文件只产生 O(N) 的日志写入和固定两次完整报告写入。
...
若移动尚未发生，表现为“源仍是普通文件且目标不存在”，该意图会安全忽略。源和目标同时存在、同时缺失、变成符号链接或目录、文件状态不匹配以及任何路径越界都会停止恢复并保留日志，等待人工检查。只要整理仍在当前 Windows 会话的任一进程中运行，报告读取就只返回基线内容，不消费正在追加的日志；进程被强制结束后，系统自动释放互斥体，下一次读取才进入恢复。

撤销使用同一套文件状态校验，但不依赖内存中的预览。整批记录会先完成预检，每项只能处于“待撤销”或“已恢复”状态；随后每次移动前后再次复核。进程中断后重新撤销时，已恢复项计为跳过，其余项继续执行。两边同时存在或同时缺失不会被当成已完成。

## 故障验证边界

边界测试使用固定随机种子，因此覆盖面可以扩展，但任意失败都能通过样本编号稳定复现。文件名生成器同时覆盖 Windows 保留字符、CJK、假名、组合字符、控制符、方向覆盖符和不成对代理字符；所有进入身份、查询、目标目录和报告的文本必须可严格编码为 UTF-8。会影响文字方向或无法形成 Unicode 标量的字符在归一化阶段替换，不改变普通标题内容。

EPUB 测试从一个有效归档生成多个截断点，并补充随机字节与解压上限样本。封面、身份和内容提示读取器必须返回有界结果或 `None`，不能让损坏文件逃出单文件解析边界。Sidecar 测试连续发送畸形 JSON、非法根类型和参数类型，随后发送合法 `ping` 与 `shutdown`，用于确认协议错误不会破坏后续会话。

文件事务测试在批次每个移动位置注入权限失败，同时覆盖移动已经完成但调用随后报错、部分报告重写失败、另一 Python 进程读取活动报告、进程强制终止和恢复后撤销。C# 集成测试则启动真实 Python 进程、强制结束并调用 `RestartAsync`，确认新进程完成版本握手与 `bootstrap`。这些测试只使用临时合成文件，不接触真实书库或在线元数据。
...
- `ViewModels/WorkspaceLayoutController.cs` 与 `ActivityLayoutController.cs` 只根据窗口有效宽高返回布局描述，不读取 UI 状态，也不改变分类计划或报告选择。
- 宽度达到 1280 有效像素时保持展开导航和侧边详情；紧凑窗口优先给结果表格留出宽度，并通过显式按钮打开详情。
- 紧凑详情使用 `SplitView` 覆盖层并支持轻触外部关闭；详情失败保留可见重试入口，不用模态对话框阻断结果浏览。
- 1024×700 是完整工作区的紧凑验证基线；更小或更低尺寸会缩减内边距并启用工作区纵向滚动。报告页低于 960 有效像素时按历史、报告条目、会话活动顺序纵向展开；每个列表仍使用有界视口保持虚拟化，列表选择和完整报告范围保持不变。
- `LN_SELECTOR_WINUI_TEST_WINDOW_SIZE` 只供自动化冒烟提供经过范围校验的启动尺寸，不是用户设置，也不改变持久化窗口状态。
- 统计条目共享一个内容面，结果和详情共享一个工作面；报告摘要保留重点高光，历史、条目和日志使用低层玻璃表面，普通层级依靠间距、底色和细分隔线表达。
...
7. 在暂存发布目录执行真实启动、完整外观和 1024×700 紧凑详情冒烟。
