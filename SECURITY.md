...
LightNovelSelector 在本地读取文件名、必要的内容提示和完整文件指纹，用于生成分类计划、识别重复项和执行撤销。开启在线识别时，只发送标题或系列查询，不上传小说文件；远程 JSON 和封面响应均设有体积上限。

分类报告包含原始路径和实际目标路径。撤销前会校验报告版本、应用标识、报告所在目录及全部路径边界，只允许在报告生成时的分类根目录内恢复文件。请勿手动修改、移动或公开上传分类报告。
