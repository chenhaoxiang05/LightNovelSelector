...
set "APP_EXE_NAME=LightNovelSelector-UI-test-v%APP_VERSION%-%BUILD_STAMP%"

if not exist "%~dp0build\ui_test_spec" mkdir "%~dp0build\ui_test_spec"
if not exist "%~dp0dlist" mkdir "%~dp0dlist"

echo Building %APP_EXE_NAME%.exe into UI_test\dlist...
"%PYTHON_EXE%" -m PyInstaller ^
  --noconfirm ^
  --clean ^
  --onefile ^
  --windowed ^
  --workpath "%~dp0build\ui_test_pyinstaller" ^
  --specpath "%~dp0build\ui_test_spec" ^
  --distpath "%~dp0dlist" ^
  --name "%APP_EXE_NAME%" ^
...
echo.
echo Done: %~dp0dlist\%APP_EXE_NAME%.exe
