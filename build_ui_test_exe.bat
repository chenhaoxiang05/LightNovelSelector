...
set "APP_EXE_NAME=LightNovelSelector-ui-v%APP_VERSION%-%BUILD_STAMP%"

if not exist "%~dp0build\ui_spec" mkdir "%~dp0build\ui_spec"
if not exist "%~dp0dlist" mkdir "%~dp0dlist"

echo Building %APP_EXE_NAME%.exe into ui branch dlist...
"%PYTHON_EXE%" -m PyInstaller ^
  --noconfirm ^
  --clean ^
  --onefile ^
  --windowed ^
  --workpath "%~dp0build\ui_pyinstaller" ^
  --specpath "%~dp0build\ui_spec" ^
  --distpath "%~dp0dlist" ^
  --name "%APP_EXE_NAME%" ^
